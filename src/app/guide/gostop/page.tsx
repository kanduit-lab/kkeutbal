...
const MULTIPLIER_ROWS: readonly MultiplierRow[] = [
  ...
  { source: '3고부터', effect: '점수 전체에 배수가 붙어요. 고를 부를수록 커지지만 뒤집히면 그만큼 크게 잃어요' },
  ...
  { source: '멍박', effect: '기본 룰에서는 꺼져 있어요 (방장이 켤 수 있음)' },
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-5 pb-16 pt-8 lg:px-8 lg:pt-12">
      <GuideHeader
        ...
        emoji="🌸"
        title="고스톱"
        description="바닥 패와 맞춰 광·열끗·띠·피를 모으고, 점수가 나면 고와 스톱 중 하나를 선택해요"
      />
        ...
        <div className="space-y-8 lg:col-span-9">
          <section id="flow" className="scroll-mt-6 space-y-3">
            ...
            <Panel>
              <ol className="list-inside list-decimal space-y-2 text-sm text-text">
                <li>각자 패를 받고, 바닥에도 몇 장이 깔려요</li>
                <li>내 패와 같은 월의 바닥 패가 있으면 짝을 맞춰 가져와요</li>
                <li>광·열끗·띠·피를 모아서 점수가 나면 고를 부를지 스톱할지 선택해요</li>
                <li>스톱을 선언하면 그 자리에서 점수가 확정돼요</li>
              </ol>
            </Panel>
          </section>
          ...
          <section id="score" className="scroll-mt-6 space-y-3">
            ...
            <p className="text-xs text-muted">
              기본 룰에서는 3점 이상 모아야 스톱을 선언할 수 있어요
            </p>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Panel className="space-y-1.5">
                ...
                <p className="text-sm text-muted">
                  점수가 났을 때 판을 계속 이어가는 선언이에요. 더 큰 점수를 노릴 수 있지만, 상대에게
                  역전당하면 그만큼 손해가 커져요
                </p>
              </Panel>
              <Panel className="space-y-1.5">
                ...
                <p className="text-sm text-muted">
                  그 자리에서 판을 끝내고 지금까지 모은 점수를 그대로 확정해요
                </p>
              </Panel>
            </div>
          </section>
