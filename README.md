...
| 💳  | **가상 크레딧**         | 방 생성 때 세션 칩 또는 계정 크레딧을 선택. 계정 모드는 바이인 잠금·취소·최종 정산을 변경 불가 거래로 남김. 현금 가치·충전·환전·출금 없음 |
| 🔐  | **검증 가능한 섯다 딜** | 선택한 방에서 server/client commit-reveal로 20장을 셔플. 내 패만 서버에서 받고, 종료 뒤 전체 덱을 재계산해 감사                           |
...
| 🔮  | **족보 판독**           | 수동 피커 + Anthropic/Gemini 사진 인식, 서열·승률·확률 통계                                                                               |
...
| Vision | `@anthropic-ai/sdk` + `@google/genai`, 환경변수 API key 준비 상태를 관리자 콘솔에 표시하고 공급자·모델·활성화 관리                       |
...
인코딩한 값을 `DATABASE_CA_CERT_BASE64`에 넣습니다.

```powershell
[Convert]::ToBase64String([IO.File]::ReadAllBytes('C:\path\to\prod-supabase.cer'))
```

처음 실행할 때 로그인 화면에서 **초기 관리자 만들기**를 선택하고, 서버 콘솔에 표시된
1회용 설정 코드를 입력합니다. 코드는 10분 뒤 만료되며, 화면을 다시 열면 새 코드가 콘솔에
출력됩니다. 검증 후 만든 첫 계정만 관리자이며, 이후 내부 계정 가입은 `/admin`에서 관리자가
발급한 가입코드가 필요합니다.
