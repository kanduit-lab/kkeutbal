...
> 판돈 없이 즐기는 화투(섯다·고스톱) 실시간 판(板) 기록·베팅·랭킹·족보 트래커

MT나 모임에서 친구들끼리 **돈 안 걸고** 포커·섯다·고스톱을 하면, 칩을 공용으로 쓰다 보니
누가 얼마 땄는지, 오늘 판의 승자가 누군지 기록이 안 남는다. **끗발**은 그 문제를 푼다.

한 명(딜러/감사)이 또는 각 플레이어가 자기 폰으로 베팅·결과를 실시간 입력하면,
방 안 모두의 화면이 즉시 동기화되고, 판이 끝나면 개인 예산·승패·랭킹이 자동 집계된다.
섯다·고스톱 **족보(끗) 판독**은 카드를 손으로 고르거나 사진 한 장으로 즉시 확인한다.
...
| 기능 | 설명 |
|------|------|
| 🎴 **실시간 판(Room)** | 방 코드/QR로 입장, WebSocket으로 베팅·액션이 전원 화면에 즉시 반영 |
| 💰 **개인 예산 관리** | 참가자별 바이인/예산 설정, 잔액·손익 실시간 추적, 한도 경고 |
| 🕹️ **원격 베팅** | 각자 폰에서 콜/레이즈/폴드·판돈 입력, 딜러 승인 or 신뢰 모드 |
| 🏆 **승패 랭킹** | 세션·누적 순손익, 승률, 최대 팟, MVP·호구 등 재미 지표 |
| 🔮 **족보 Advisor** | 섯다·고스톱 끗/점수 판독 — 수동 카드 피커 + 카메라 사진 인식(Claude vision) |
