...
핵심 기능과 순수 엔진 검증은 구현돼 있다. 남은 것은 **실배포 라이브 경로 검증, 전체 사용자
흐름 E2E 확장, UI 회귀 가드, 이후 DB 마이그레이션 드리프트 방지**다.
...
남은 공백은 대부분 **검증·설정**이지만 전부는 아니다. 2026-07-29 수동 UI 감사에서 구현
결함 두 건이 나왔다. 좌석이 펠트 타원 밖으로 넘치는 배치 결함(`game-table.tsx`의 좌석
타원이 `inset-[7%]`로 그린 펠트와 어긋남), 그리고 고스톱 방에서 판이 도는 동안 딜러가 아닌
참가자에게 아무 화면도 안 뜨는 결함(`room-client.tsx`의 `isBettingGame`이 고스톱을 제외해
`ActionBar`가 안 붙고, 점수 입력은 `DealerPanel` 전용)이다. 둘 다 2026-07-29에 수정했다.
좌석 반지름은 `--felt-inset` 상수 하나로 펠트 타원과 묶었고, 고스톱 비딜러 화면에는
`GostopWaitPanel` 대기 표면을 붙였다. 판과 판 사이(`currentRound`가 null인 구간)의 비딜러
화면은 아직 비어 있다.

자동 스위트가 이걸 못 잡은 건 우연이 아니다. 단위 테스트는 `vitest.config.ts`가
...
그래도 우선순위는 그대로다. UI 결함은 화면을 열면 드러나고 사람이 눈으로 교정할 수 있지만,
라이브 경로는 실배포에서 돌려보기 전까지 드러나지 않고 대체 경로도 없다. 순서는 전제의
원칙대로 "수동 대체 경로가 없는 것부터". 이 검증 갭을 메우는 작업을 포함해 실행 항목의
상세는 `TODO.md`가 소유한다.

### P0 — 라이브 경로 증명 (죽으면 MT 당일 앱 자체가 없다)
...
2. **실배포 스모크 1회** — 배포된 앱에서 실기기 2대로 전체 플로우 완주: 로그인 → 방 생성 →
   ...
   돌아간 적이 아직 한 번도 없어서 정보량이 가장 큰 단일 검증이다. Broadcast 왕복 p95
...
1. **Playwright E2E 전체 흐름 확장** — 공개 화면 8개는 DB 없이 실행된다. 인증 후 방
   수명주기 spec은 이미 있으나 전용 계정 2개와 `E2E_ENABLE_ROOM_LIFECYCLE=true` 없이는
   skip된다. 격리된 가입코드/로그인 fixture를 마련해 2컨텍스트 동기화, 재접속 복원, 정산
   흐름을 실제로 돌린다.
2. **UI 회귀 가드 도입** — 역할별 렌더링과 데스크톱 폭 배치를 자동으로 잡는 층이 지금 없다.
   2026-07-29 감사 결함이 여기로 샜다.
3. **staging/preview 채널 전환** — `ENV_FILE_BASE64` 시크릿 구성 후 `.deploy.yml`의
...
1. **라이브 경로 미증명** — 배포+인증+실시간을 엮은 엔드투엔드가 실배포 상태로 돈 적이 없다.
   여기가 죽으면 종이 기록으로 회귀하는 것 말고 대체가 없다 (P0-2).
...
5. **UI 회귀 재발** — 자동 스위트가 화면을 렌더링하지 않으므로 이번에 고친 배치·역할
   게이팅 결함이 다시 들어와도 CI는 초록이다. 사람이 눈으로 보면 잡히니 위 네 항목보다
   아래지만, 지금은 탐지가 수동 감사에만 걸려 있다 (P2-2).
