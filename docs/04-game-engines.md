...
엔진이 `{ kind: 'replay' }`를 내는 경우(구사 보유, 동률 `tieBreak: 'replay'`)와 고스톱 나가리는
**승부가 나지 않은 판**이다. 이때 판돈은 **다음 판으로 이월**한다 — 실제 섯다·고스톱에서 구사
재경기와 나가리는 판돈을 그대로 두고 다시 돌리는 것이 관례이고, 환불하면 "먼저 지른 사람이
손해를 안 본다"는 판의 긴장이 사라진다.

무효화(`voidRound`)는 사유에 따라 판돈 처리가 갈린다:

| 사유 | 판돈 | 이유 |
|------|------|------|
| `재경기` | **다음 판으로 이월** | 승부가 안 난 판. 건 사람은 계속 걸어둔 상태로 다시 돌린다 |
| `오입력` | 전액 환불 | 판 자체가 성립하지 않았다. 없던 일로 되돌린다 |
| `패 노출` | 전액 환불 | 같음 |

**이월의 구현은 "환불 후 재징수"다.** 무효화 시점에는 사유와 무관하게 원장에 `correction`
행을 넣어 칩을 돌려준다. 대신 `재경기`였고 그 판이 `playing`이었으면 사람별 기여액을 그 판의
`rounds.result.carry`에 남기고, **다음 `startRound`가 그 몫을 새 판의 `bet` 원장 행으로 다시
걷는다**. 이렇게 나눈 이유:

- 무효화 직후 방이 정산되거나 아무도 다음 판을 안 열어도 칩이 공중에 뜨지 않는다. 이월은
  다음 판이 실제로 열릴 때만 성립한다.
- 팟 계산(`getRoundPot`)은 여전히 **판 단위**로 닫혀 있다. 판을 가로지르는 팟 개념을 만들지
  않으므로 정산 보존식(`netTotalInRoom === 0`)이 건드려지지 않는다.
- 재징수 대상은 **새 판의 참가자이면서 이월 몫이 있는 사람**뿐이다. 그 사이 나갔거나 관전으로
  바뀐 사람 몫은 환불된 채로 남는다(그만큼 이월 판돈이 줄어든다).

재징수는 `bet_actions` 행을 만들지 않는다 — 기여도(`round-bet-state.ts`)는 `bet_actions`에서,
팟(`getRoundPot`)은 원장에서 나오므로, 이월 판돈은 **팟에는 들어가고 아무의 콜 의무도 만들지
않는다**. 새 판은 전원 기여 0에서 다시 시작하고 팟만 이월액에서 출발한다(앤티와 같은 모양).

미소비 이월액은 `RoomSnapshot.carriedPot`으로 나가고 로비가 "이월 판돈"으로 보여준다.
