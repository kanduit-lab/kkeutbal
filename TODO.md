...
- [ ] **테스트 계정 정리**: `testadmin1`/`testadmin2`/`testadmin3`을 배포 전에 없애거나 권한을 내린다
  ...
  - 참조: 이 계정을 참조하던 e2e·통합 테스트를 모두 제거해서(2026-08-09) 지금은 어떤 코드도 쓰지 않는다 — 삭제를 막는 것이 없다. 셋 다 `is_admin = true`다([`docs/12-handoff.md`](docs/12-handoff.md) "참고 — 로컬 개발 환경")
...
- **테스트는 한 층뿐**: 단위·컴포넌트(`pnpm test` — 순수 함수는 node, `test/dom/**`은 jsdom). 브라우저(e2e)와 실제 DB(Server Action 통합) 층은 2026-08-09에 제거했다.
